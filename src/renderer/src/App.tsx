...
function ChatWorkspace({
  ...
  const runChatTool = async (
    ...
    setToolStatus(
      `${command.invocation.toolId}.${command.invocation.action} completed and its visible result entered working experience.`
    );
...
function ImaginationWorkspace({ brain, onToast }: { brain: BrainDocument; onToast: (message: string) => void }) {
  ...
  const downloadOutput = () => {
    ...
    const extension = mimeType.includes("png") ? "png" : mimeType.includes("wav") ? "wav" : "bin";
    const anchor = document.createElement("a");
    anchor.href = dataUrl;

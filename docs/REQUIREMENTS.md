...
- [ ] ARM64 Windows package builds in clean native CI. The workflow builds a native ARM64 Electron shell with an x64 PyTorch/PyInstaller worker for Windows 11 emulation. Run 30066558290 passed every source, built-UI, worker, and package stage, and its installed worker passed the comprehensive neural exercise. The NSIS install then lacked only the top-level desktop executable while retaining the full resources tree. Run 30067949274 proved that switching the entire installer to ZIP extraction was pathologically slow on both architectures and was cancelled. Run 30069924407 showed that the 7z staging archive itself lacked the executable, so the final non-rerun fix embeds the verified ARM64 executable directly from electron-builder's known `release/win-arm64-unpacked` application directory. Its x64 companion installed and launched successfully but hit a transient Playwright CDP handshake timeout, now covered by one CI-only whole-test retry.
...
- [Windows run 30069924407](https://github.com/Eabusham2/Omni-AGI/actions/runs/30069924407): ARM64 again passed every gate through native package creation and silent installation, which proved the custom staging copy could not recover a file absent from the archive itself. The custom include now embeds the already-verified `release/win-arm64-unpacked/Omni AGI Studio.exe` directly. x64 reached a live CDP WebSocket before Playwright's 120-second handshake timed out; CI now retries the isolated end-to-end test once.

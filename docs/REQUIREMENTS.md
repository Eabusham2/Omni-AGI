...
- [ ] ARM64 Windows package builds in clean native CI. The workflow builds a native ARM64 Electron shell with an x64 PyTorch/PyInstaller worker for Windows 11 emulation. Run 30066558290 passed every source, built-UI, worker, and package stage, and its installed worker passed the comprehensive neural exercise. The NSIS install then lacked only the top-level desktop executable while retaining the full resources tree. Packaging now forces NSIS direct ZIP extraction instead of the failing 7z staging/copy path and checks the executable both before and after worker exercise; per user direction this final fix was not rerun.

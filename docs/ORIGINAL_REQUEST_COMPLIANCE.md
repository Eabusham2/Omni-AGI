...
| Ensure it really uses 1.58-bit rather than silently using 16-bit forward weights | Implemented | Strict ternary audit and packed two-bit shards fail closed on dense eligible projections | `engine/omni_core/ternary_packing.py`; `engine/tests/test_ternary_packing.py` |
...
| Use vector-symbolic architectures | Implemented | VSA/HDC binding, bundling, permutation, approximate recall, and assembly formation | `engine/omni_core/vsa.py`; `engine/tests/test_memory_modalities.py` |
...
| Remove fixed parallel-thought and recurrent-recall counts, not only their UI controls | Partial | Chat still uses a hardware-tier branch budget and spreading activation still uses a four-hop propagation loop | `engine/omni_core/brain.py`; `engine/omni_core/vsa.py` |
| Make curiosity, pondering, imagination, and motivation organic and unprompted | Implemented | Prediction error, novelty, uncertainty, learning progress, spike/liquid state, and unfinished activity drive prompt-free idle cycles and typed proposals | `engine/omni_core/brain.py`; `src/main/idleCognitionScheduler.ts`; `engine/tests/test_brain.py`; `tests/neuralFeedbackIdle.test.ts` |
| Let idle cognition organically ask the user a question | Not implemented | Idle cycles can rehearse and propose external actions, but they do not append a spontaneous `talk` message to the continuous chat | `engine/omni_core/brain.py`; `src/main/idleCognitionScheduler.ts` |
...
| Add a real working memory/context window | Implemented | Hardware-sized recent token/sensory context, latent slots, recurrent liquid state, active assemblies, rehearsal, decay, and eviction | `engine/omni_core/brain.py`; `engine/omni_core/model.py`; `tests/stableBrainInspection.test.ts` |
...
| Export/share the current brain and its original copy | Implemented | Portable and local-reference `.omni` bundles include checksum-bound current/origin state and packed ternary generations | `src/main/brainRepository.ts`; `tests/brainRepository.test.ts` |
| Restore snapshots and the immutable origin | Implemented | Snapshot, restore, origin preservation, and corrupt-state rejection are covered | `src/main/brainRepository.ts`; `tests/brainRepository.test.ts` |
...
| Train on the entire requested dataset | Implemented | Every valid manifested record is visited in every requested epoch, with a committed cursor and explicit rejected-record report | `src/main/dataIngestion.ts`; `engine/omni_core/datasets.py`; `tests/dataIngestion.test.ts`; `engine/tests/test_datasets.py` |
...
| Never silently skip data | Implemented | Coverage records every processed or rejected unit and persists errors/cursors | `src/main/dataIngestion.ts`; `tests/dataIngestion.test.ts` |
...
| Let the brain naturally choose whether to talk, use a tool, imagine, agent, ponder, learn, evolve, or stop | Partial | The neural head scores all eight choices and materializes imagination, agent, evolution, stop, and internal actions. The generic tool ID/action/argument bridge is provisional pending its final worker fixture | `engine/omni_core/model.py`; `engine/omni_core/brain.py`; `engine/omni_core/starter.py` |
...
| File, PowerShell, coding, web, and browser tools | Partial in natural chat | Structured protocols, visible execution, cancellation, and permission enforcement are implemented. Learned generic tool materialization is still a provisional current-revision integration | `tools/catalog.json`; `src/main/toolExecutor.ts`; `tests/toolExecutor.test.ts` |
| Full interactive browser automation | Partial | The current browser protocol safely renders a script-disabled public-page snapshot and returns text, links, and a screenshot; it does not click/type in a signed-in session | `src/main/toolExecutor.ts`; `tests/toolExecutor.test.ts` |
...
| Imagine organically in real time while writing/talking | Implemented | Organic typed imagination can begin mid-turn; ordered image/audio/video previews stream while generation is running | `engine/omni_core/brain.py`; `engine/omni_core/modalities.py`; `engine/worker.py`; `tests/actionProtocol.test.ts`; `engine/tests/test_worker.py` |
...
| Fork subagents, let them work separately, then merge | Implemented | Isolated copy-on-write branches return reviewed assemblies, evidence, files, artifacts, and replay examples without whole-model averaging | `src/main/brainService.ts`; `src/main/toolExecutor.ts`; `tests/toolWorkflows.test.ts` |
...
| Improve source code in isolation | Partial | Source candidates use Git worktrees, allowlisted checks, exact diff hashes, and promotion/rollback records, but the proposal action does not autonomously author the code change | `src/main/toolExecutor.ts`; `src/main/evolutionController.ts`; `tests/toolWorkflows.test.ts` |
| Under Full Authority, rebuild/install a passing source candidate and restart into it | Not implemented | Current source promotion commits the verified change but does not replace or restart the running packaged binary | `src/main/evolutionController.ts`; `src/main/toolExecutor.ts` |
...
| Review, approve, stop, and roll back evolution from the Run interface | Partial | Main/preload APIs implement the lifecycle, but the renderer does not yet expose the candidate controls | `src/main/evolutionController.ts`; `src/preload/index.ts`; `src/main/ipc.ts` |
| Automatically rewrite arbitrary architecture and migrate incompatible tensor shapes | Not implemented | Stable v1 rejects architecture candidates because safe shape migration is not available | `src/main/evolutionController.ts`; `engine/omni_core/evolution.py`; `engine/tests/test_neural_evolution.py` |
